...
function uuidV5FromString(name, namespace = '6ba7b811-9dad-11d1-80b4-00c04fd430c8') {
...
exports.handler = async (event) => {
  try {
    const sb = getClient()
    const vid = event.headers['x-visitor-id'] || event.headers['X-Visitor-Id']
    if (!vid) return { statusCode: 400, body: 'Missing visitor id' }
    const ownerId = uuidV5FromString(String(vid))
    ...
    const action = body.action || event.queryStringParameters?.action || 'upsert'
    ...
    if (method === 'POST' && action === 'upsert') {
      ...
      if (id) {
        const { data, error } = await sb
          .from('songs')
          .update({ title, key, bpm, body_chordpro, visitor_id: vid, owner_id: ownerId, updated_at: new Date().toISOString() })
          .eq('id', id)
          .eq('visitor_id', vid)
          .select('id')
          .single()
        ...
        const { data, error } = await sb
          .from('songs')
          .insert({ title, key, bpm, body_chordpro, visitor_id: vid, owner_id: ownerId })
          .select('id')
          .single()
        if (error) throw error
        return json({ id: data.id })
